...
class AdvancedVisualizer:
    ...
    def plot_violin_comparison(self, filename='violin_comparison.png'):
        ...
        for i, (metric, label) in enumerate(zip(metrics, labels)):
            ...
            if key in self.stats:
                ...
                p = d.get('p_fdr', d.get('p_value_fdr', 1))
                sig = '***' if p < 0.001 else '**' if p < 0.01 else '*' if p < 0.05 else 'n.s.'
    ...
    def plot_mattr_comparison(self, filename='mattr_comparison.png'):
        if 'mattr_score' not in self.df.columns:
            print("   -> Keine MATTR-Scores für Visualisierung vorhanden.")
            return
        plt.figure(figsize=(8, 5))
        data = [
            self.df[self.df['condition'] == 'normative']['mattr_score'].dropna(),
            self.df[self.df['condition'] == 'disability']['mattr_score'].dropna(),
        ]
        bp = plt.boxplot(data, labels=['Normativ', 'Disability'], patch_artist=True)
        bp['boxes'][0].set_facecolor('#e3f2fd')
        bp['boxes'][1].set_facecolor('#f3e5f5')
        plt.ylabel('MATTR-Score (Fenster = 50)')
        plt.title('Lexikalische Diversität nach Bedingung')
        plt.ylim(0.80, 0.92)
        if 'mattr' in self.stats:
            m = self.stats['mattr']
            plt.text(0.5, 0.95,
                     f"Delta = {m['delta']:.4f}, p = {m['p_value']:.4f}",
